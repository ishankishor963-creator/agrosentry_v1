 ...
 * - Smooth numerical counter ticker for real-time telemetry
 * - Full Light Sensor integration (Lux, PPFD, DLI, spectral needle)
 * - Zero-latency WebRTC camera reticle scanner
 * - Modern async API integration
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (initialActiveNav) {
    setTimeout(() => updateNavIndicator(initialActiveNav), 50);
  }
  const initialActiveSubTab = document.querySelector('.sub-tab-btn.active');
  if (initialActiveSubTab) {
    setTimeout(() => updateSubTabIndicator(initialActiveSubTab), 50);
  }
  ...
  window.addEventListener('resize', () => {
    ...
    if (activeSubTab) updateSubTabIndicator(activeSubTab);
  });
  ...
  // ----------------- Live Telemetry & Light Sensor -----------------
  async function fetchTelemetry() {
    try {
      const resp = await fetch('/api/sensors');
      const data = await resp.json();

      // Animate Dashboard values smoothly
      ...
      // Animate Sensors view values smoothly
      ...
      // Save previous values for next interpolation
      ...
      // Light Sensor Condition & Spectrum Needle Position
      ...
      // Smoothly reposition spectral needle across 5k -> 80k Lux scale
      if (spectrumNeedle) {
  ...
  async function fetchAlerts() {
    try {
      ...
      // Solar Heat / Radiation Scorch Alert
